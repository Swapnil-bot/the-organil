...
async function registerUser(params) {
    ...
    // Check if user exist
    let user = await Admin.findOne({ where: { emailID: params.email } }).catch((error) => { return { error } });
    if (user) {
        return { error: "Admin already exists", status: 409 };
    }
    // hash Password  
    ...
    // Insert Data Into DB
    let insert = await Admin.create(data).catch((error) => { return { error } });
...
async function loginUser(params) {
    ...
    let isExist = await Admin.findOne({ where: { emailID: params.email } }).catch((error) => { return { error } });
...
async function fogetPassword(params) {
    ...
    if (!insert || (insert && insert.error)) {
        return { error: "Admin can't Login, Please Try Again", status: 500 }
    }
    // Send Email to user
    let data = {
        from: mailCrediential.user,
        to: params.email,
        subject: "Forgot Password OTP For Admin",
...
async function resetPassword(params) {
    ...
    let update = await Admin.update({ password: hashPassword, otp: "" }, { where: { id: user.id } }).catch((error) => { return { error } });
    if (!update || (update && update.error)) {
        return { error: "Internal Server Error", status: 500 }
    }
    // Return Response
...
async function changePassword(params, userData) {
    ...
    let user = await Admin.findOne({ where: { id: userData.id } }).catch((error) => { return { error } });
    if (!user || (user && user.error)) {
    ...
    let updatedUser = await Admin.update({ password: newPassword }, { where: { id: userData.id } }).catch((error) => { return { error } })
...
async function logOut(userData) {
    ...
    // Check if User Exist 
    let checkExist = await Admin.findOne({ where: { id: userData.id } }).catch((error) => { return { error } });
